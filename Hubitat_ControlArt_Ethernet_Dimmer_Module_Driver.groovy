 ...
 */
metadata {
  definition (name: "Controlart - Ethernet DIMMER Module", namespace: "VH", author: "VH", vid: "generic-contact") { 
...
command "connectionCheck"
  ...
  preferences {
        ...
        input name: "outputs", type: "string", title: "How many Dimmers " , defaultValue: 9
...
@Field static Integer checkInterval = 200
...
def createchilds() {
	...
	if (!cd) {
        ...
        log.info "added dimmer # " + i + " from " + state.outputcount            
...
def parse(msg) {
    ...
    //macadress
    if ((newmsg2.contains("macaddr") && (newmsg2.length() == 18))) {
        //log.info "mac completa = " + newmsg2
        mac = newmsg2
        log.info " mac = " + mac

        newmac = (mac.substring(7));
        log.info " newmacsubstring = " + newmac
        newmac = (newmac.replaceAll(",","0x"));
        log.info " replace ,  = " + newmac        
        newmac = (newmac.replaceAll("-",",0x")); 
    ...
    //getstatus
    if ((newmsg2.contains("setcmd") && (newmsg2.length() == 61))) {
